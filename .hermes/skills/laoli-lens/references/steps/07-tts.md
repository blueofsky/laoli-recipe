...
**注意**：
- 配音时长由TTS根据台词字数和speed参数自动生成，不需要预设时长
- 气口（停顿）通过MiniMax行内标签实现：`(breath)` `(sighs)` `(inhale)` `(exhale)`
- 第8步会通过变速对齐视频和配音
...
配音文件生成完毕后（无配音分镜也需有记录），必须通过 Task tool 调用 `laoli-lens-reviewer` 进行评审（将触发 L3 深度验证：配音参数合规）。仅当评审返回 **✅ 通过** 后，方可告知 Owner 本步完成、进入下一步。**未完成评审 = 本步未完成。**
