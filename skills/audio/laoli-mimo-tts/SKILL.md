...
当用户请求语音合成时，按以下规则选择模式：

1. **用户附带了音频文件** → 音色克隆模式（`mimo_tts_voiceclone.py --voice-file <文件>`）
2. **用户提到了预置音色名**（冰糖/茉莉/苏打/白桦/Mia/Chloe/Milo/Dean）→ 预置音色模式（`mimo_tts.py --voice <音色名>`）
3. **用户描述了音色特征**（如"磁性男声""甜美女声""沧桑老者"）→ 音色设计模式（`mimo_tts_voicedesign.py --context "<描述>"`）
4. **用户没指定音色** → 默认用「冰糖」（预置音色，最快）

### 用户说法示例 → 对应模式

| 用户说法 | 模式 | 关键参数 |
|---|---|---|
| 「用冰糖的声音说 xxx」 | 预置音色 | `--voice 冰糖` |
| 「用茉莉朗读这段话」 | 预置音色 | `--voice 茉莉` |
| 「用磁性低沉的男中音说 xxx」 | 音色设计 | `--context "磁性低沉男中音..."` |
| 「帮我用东北大哥的嗓音配音」 | 音色设计 | `--context "东北大哥..."` |
| 「[附带音频] 用这个声音说 xxx」 | 音色克隆 | `--voice-file <文件>` |
| 「朗读这段话」（无音色要求） | 预置音色（默认冰糖） | `--voice 冰糖` |

### 自然语言风格控制（三种模式均支持）

在任何模式下，都可以通过 `--context` 参数添加风格指令：

```bash
# 预置音色 + 风格控制
python3 mimo_tts.py --text "..." --voice "冰糖" --context "用轻快上扬的语调，带着激动"

# 音色克隆 + 风格控制
python3 mimo_tts_voiceclone.py --text "..." --voice-file sample.wav --context "用低沉缓慢的语调"
```
...
```bash
python3 mimo_tts.py --text "(唱歌)两只老虎，两只老虎，跑得快" --voice "冰糖"
```
...
| `MIMO_BASE_URL` | API 地址，默认 `https://api.xiaomimimo.com/v1`   | 否   |
...
```bash
python3 {baseDir}/scripts/mimo_tts.py \
  --text "你好，今天天气真不错。" \
  --voice "冰糖"
```
...
```bash
python3 {baseDir}/scripts/mimo_tts_voicedesign.py \
...
```bash
python3 {baseDir}/scripts/mimo_tts_voiceclone.py \
...
bash {baseDir}/scripts/feishu_send_audio.sh /tmp/mimo-v2.5-tts/voice.wav open_id ou_xxxxxx
```
...
bash {baseDir}/scripts/feishu_send_audio.sh /tmp/mimo-v2.5-tts/voice.wav chat_id oc_xxxxxx
```
