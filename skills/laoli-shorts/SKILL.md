...
>
> **评审机制**：详见 `references/review-guide.md`。每步完成后须在此机制下提交评审（写入项目下的 `文案/评审记录.md`）。
>
> **执行规则**：每步完成后展示产出物并提交评审（写入项目下的 `文案/评审记录.md`），然后**告知 Owner 安排审核**。评审通过后方可进入下一步。
>
> 收到 Owner 转达的评审结果后：
> - **✅ 通过** → 继续执行下一步
> - **⚠️ 需确认** → 等待 Owner 决策，收到结果后按 ✅ 或 ❌ 处理
> - **❌ 不通过** → 读取 `文案/评审记录.md` 中的修改意见，修复后重新提交评审（设 ⏳ 待评审，`修改时间` 填入当前时间 `{YYYY-MM-DD HH:mm}`）
...
> ⚠️ **执行前必读**：读取 `references/profiles/<default_profile>`（由 EXTEND.md 指定，默认为 `history-oil.md`），了解本方案的调性、受众、视觉风格、制作规范和风格铁律。所有台词创作必须遵守该文件中的"风格铁律"，所有 IMAGE PROMPT 末尾必须追加该文件中的风格后缀。
...
> ⚠️ **第5步特别提醒**：视频生成成本高，分镜图必须经评审通过后，才能执行第6步生成视频。

### 第1步：选题大纲

**输入**：用户给的主题，或基于方案定位的建议

**输出**：确定项目名后，创建 `<projects_dir>/[项目名]/` 目录结构（含文案/、素材/图片/、素材/封面/、素材/视频/、素材/配音/、素材/音效/、素材/定妆/、素材/BGM/、成品/），然后创建两个文件：

1. `文案/创作底本.md`：写入选题大纲
2. `文案/评审记录.md`：按 `references/review-guide.md` 中的「评审记录结构」创建完整评审记录（含总览表、各步骤 Round 占位等全部结构），填写项目名、创建时间、步骤 1的Round等信息
...
**产出确认**：在`文案/评审记录.md`中写入提交说明（详见 `references/review-guide.md` 中的「提交说明格式」），将总览表状态设为 ⏳ 待评审、`修改时间` 填入当前时间 `{YYYY-MM-DD HH:mm}`，等待 Owner 安排审核。
...
- **定妆照提示词**在第4步直接读取并使用，生成后保存为 `素材/定妆/ref_角色ID.jpg`，第5步生成分镜图时作为角色参考图传入
...
展示 `素材/定妆/` 下所有已生成图片，按以下清单自检：
  ...
  - **发现问题**：记录有问题的文件名，重新生成对应图片，重新自检
  - **自检通过后**：在`文案/评审记录.md`中写入提交说明（详见 `references/review-guide.md` 中的「提交说明格式」），将总览表状态设为 ⏳ 待评审、`修改时间` 填入当前时间 `{YYYY-MM-DD HH:mm}`，等待 Owner 安排审核
...
- **输入**：`素材/视频/原始/scene0x_描述.mp4`（原始）、`素材/配音/scene0x_描述.mp3`
- **系数计算**：`setpts = 音频时长 / 视频时长`
- **处理方式**：纯变速（`setpts`），不加补帧，去掉音频轨道（`-an`）
- **输出**：`素材/视频/变速/scene0x_描述.mp4`（无声，所有分镜都有）
...
- **输入**：`素材/视频/原始/scene0x_描述.mp4`（第6步产出）、`素材/配音/scene0x_描述.mp3`（第7步产出，提供变速目标时长）
- **处理流程**：
  1. 提取：从原始分镜视频 `素材/视频/原始/scene0x_描述.mp4` 提取音频轨道，输出到 `素材/音效/原始/scene0x_描述.mp3`
  2. 变速：将上一步提取的音频按对应配音 `素材/配音/scene0x_描述.mp3` 的时长变速，输出到 `素材/音效/变速/scene0x_描述.mp3`
- **输出**：`素材/音效/变速/scene0x_描述.mp3`（每个分镜一条）
...
| 分镜 | 视频原长 | 音频时长 | 系数 | 水印处理 | 剪映补帧 | 音效提取 |
|:----|:-------:|:--------:|:----:|:---------|:--------:|:--------:|
...
- **示例**：选题"泰坦尼克号最后的电报" → `历史叙事 悬疑 管弦` / `historical narrative suspense orchestral`
...
- 10c → 生成 `素材/BGM/bgm.mp3`，同时将描述关键词存档到 `文案/配乐方案.md`
