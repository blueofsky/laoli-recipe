...
- **Use whatever image-generation tool or skill is available** in the current runtime — e.g., Codex `imagegen`, Hermes `image_generate`, `laoli-image`, or any equivalent the user has installed.
...
Concrete tool names (`imagegen`, `image_generate`, `laoli-image`) above are examples — substitute the local equivalents under the same rule.
...
- If `usage: direct` AND the chosen backend accepts reference images (e.g., `laoli-image` via `--ref`) → pass the file via the backend's ref parameter
...
If config has `preferred_layout` and `preferred_style` set (not null) → **skip confirmation**, use existing config, go to Step 5. `language` and `preferred_aspect` default to `null` (= auto-detect), so they don't block skipping.
