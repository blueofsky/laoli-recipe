...
name: laoli-caption
description: >
  语音转文字。使用 laoli caption 命令将音频转为文字。
...
```bash
laoli caption --input <path> [--output <path>] [options]
```
...
2. **调用转写**：`laoli caption --input speech.mp3 --output transcript.txt`
...
```bash
# 转写并保存到文件
laoli caption --input meeting.mp3 --output meeting.txt

# 指定中文，提高准确率
laoli caption --input speech.wav --output transcript.txt --lang zh

# 仅打印到控制台（不保存文件）
laoli caption --input audio.mp3
```
