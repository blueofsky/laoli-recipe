...
检查变速后的视频是否有平台水印（如 Veo），有则批量裁剪。

- **输入**：`素材/视频/sync/scene0x_描述.mp4`（8a的输出）
- **裁剪规格**：底部裁剪约70px，精确9:16比例（`1040x1850`，居中裁切）
- **参数**：`crop=1040:1850:20:0`（宽1040、高1850，从x=20开始）
- **输出**：`素材/视频/sync_crop/scene0x_描述.mp4`
- **仅处理有水印的分镜**：无水印的跳过，剪映直接使用 `sync/` 中的文件
...
汇总剪辑所需的所有信息，作为剪映阶段的制作依据。

**输入**：第1步选题大纲 + 第3步分镜脚本（金句）

**输出**：保存到 `<projects_dir>/[项目名]/文案/剪辑指南.md`
...
**输入**：第1步选题大纲（核心冲突/视觉符号）+ 第10步剪辑指南（封面标题/风格描述）
...
**输入**：第1步选题大纲（核心冲突/金句）+ 第3步分镜脚本（台词金句）+ 第10步剪辑指南（发布文案草稿）
