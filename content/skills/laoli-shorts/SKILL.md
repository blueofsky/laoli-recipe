...
>
> **执行规则**：每步完成后展示产出物并提交评审（写入项目下的 `文案/评审记录.md`）。由 Owner（laoli）通知 Reviewer（另一 AI Agent）审核并反馈结果给你，对有争议的事项做最终决策。评审通过后方可进入下一步。
...
**产出确认**：在`文案/评审记录.md`中写入提交说明（详见 `references/review-guide.md` 中的「提交说明格式」），设状态为 ⏳ 待评审，交由 Reviewer 审核。
...
展示 `素材/定妆/` 下所有已生成图片，按以下清单自检：
  ...
  - **自检通过后**：在`文案/评审记录.md`中写入提交说明（详见 `references/review-guide.md` 中的「提交说明格式」），设状态为 ⏳ 待评审，交由 Reviewer 审核
